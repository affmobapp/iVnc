...
//        if(canvasXOffset > 0)
//            canvasXOffset = canvasXOffset/2;
        canvasYOffset = -canvas.getCenteredYOffset();
//        if(canvasYOffset > 0){
//            canvasYOffset = (canvasYOffset - (canvas.getHeight())/2);
//        }
        canvas.computeShiftFromFullToView ();
        minimumScale = canvas.getMinimumScale();
        // ToDo for Auto scaling 1.f for fullscreen minimumScaling
        if((canvas.connection.getForceFull() == BitmapImplHint.FULL))
            scaling = minimumScale;
        else
            scaling=1.f;
        resetMatrix();
