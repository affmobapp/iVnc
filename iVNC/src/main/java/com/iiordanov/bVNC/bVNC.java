...
//    protected Database database;
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(CheckTrial.shouldLaunch(getApplicationContext(), CheckTrial.LICENSE_URI)) {
            ...
            if (extras != null)
                connection_id = extras.getLong(HomeActivity.PARAM_CONNECTION_REFERENCE);
    ...
    protected void connect(){
        if (ipText.getText().length() != 0 && portText.getText().length() != 0)
            ...
            Toast.makeText(getApplicationContext(), R.string.vnc_server_empty, Toast.LENGTH_LONG).show();
    }
...
//                            R.id.radioForceFullScreenAuto : R.id.radioForceFullScreenOn);
        fullScreenSwitch.setChecked(selected.getForceFull()==BitmapImplHint.AUTO ? false : true);
...
//          (groupForceFullScreen.getCheckedRadioButtonId()==R.id.radioForceFullScreenOn ? BitmapImplHint.FULL : BitmapImplHint.TILE));

        selected.setForceFull(fullScreenSwitch.isChecked() ? BitmapImplHint.FULL : BitmapImplHint.AUTO);
